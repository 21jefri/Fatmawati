...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['stok'])) {
        ...
        $id = htmlentities($_POST['id']);
        $dataS[] = $id;
        $sqlS = 'select*from produk WHERE id_barang=?';
        ...
        $data[] = $stok;
        $data[] = $id;
        $sql = 'UPDATE produk SET stok=? WHERE id_barang=?';
        $row = $config -> prepare($sql);
        $row -> execute($data);
    ...
    if (!empty($_GET['produk'])) {
        ...
        $data[] = $tgl;
        $data[] = $id;
        $sql = 'UPDATE produk SET id_kategori=?, nama_barang=?, merk=?, 
    ...
    if (!empty($_GET['jual'])) {
        ...
        $sql_tampil = "select *from produk where produk.id_barang=?";
    ...
    if (!empty($_GET['cari_barang'])) {
        ...
        if ($cari == '') {
            ...
            $sql = "select produk.*, kategori.id_kategori, kategori.nama_kategori
					...
					where produk.id_barang like '%$cari%' or produk.nama_barang like '%$cari%' or produk.merk like '%$cari%'";
            $row = $config -> prepare($sql);
            $row -> execute();
            $hasil1= $row -> fetchAll();
            ?>
